...
public class Graph<N,W> implements IGraph<N,W> {
  ...
  public void addEdge(INode<N> s, INode<N> d, W w){
    ...
    Node<N> dest = (Node<N>) d;
    Edge<N,W> edge = new Edge<N,W>(source,dest,w);
    if(edges.size() == 0){
      edges.append(edge);
    }
    for(int i = 0; i < edges.size(); i++){
      IEdge<N,W> thisOne = edges.fetch(i);
      if(thisOne.getSource().equals(source) && thisOne.getDestination().equals(dest) && thisOne.getWeight().equals(w)){
        break;
      }
      else{
        edges.append(edge);
